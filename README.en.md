...
3. **(Optional) Add a `DEEPSEEK_API_KEY` or `ORCAROUTER_API_KEY` secret**: Settings → Secrets and variables → Actions. This unlocks persona reviews, title enhancement, real pipeline-generated review lines for curated items, and more reliable Chinese title translation (refusal text and degenerate output fall back to the original title automatically). Without it everything still runs — rule-based scores, original titles, and Google Translate take over, and the review block simply doesn't render. The default DeepSeek model is `deepseek-v4-flash`; set a repo Variable `DEEPSEEK_MODEL` if you want a different one. Alternatively, set `ORCAROUTER_API_KEY` to run these features through [OrcaRouter](https://www.orcarouter.ai) — an OpenAI-compatible AI gateway — instead of DeepSeek, with the default model `orcarouter/auto` (override via `ORCAROUTER_MODEL`). The pipeline reads both keys from the same place, so in a GitHub Actions fork simply forward `ORCAROUTER_API_KEY`/`ORCAROUTER_MODEL` in the workflow env block the same way as their `DEEPSEEK_*` counterparts. Add `TITLE_ENHANCE_MAX_PER_RUN` too if you want to cap how many titles get rewritten per run (defaults to 30).
...
- Setting `ORCAROUTER_API_KEY` instead runs the same OpenAI-compatible LLM features through [OrcaRouter](https://www.orcarouter.ai), an AI gateway with adaptive routing and failover across many upstream models, instead of DeepSeek
- Default DeepSeek model is `deepseek-v4-flash` (DeepSeek is retiring the `deepseek-chat` alias on 2026-07-24); set a repo Variable `DEEPSEEK_MODEL` to override it. When using OrcaRouter, the default model is `orcarouter/auto`; override with `ORCAROUTER_MODEL`.
